...
const testModule = require('./TestModule')
// testModule.add(1, 2);
// console.log(testModule.test_('Hello,你好'));

/**
 * get-获取所有数据http://localhost:8989/server/getAll
 * get-根据url中的id查询http://localhost:8989/server/getById/1001, 用req.params.id, getByid?num=0&size=3用req.query.num
...
// var pool = mysql.createPool({//配置连接池
//     host: 'localhost',
//     user: 'root',
//     password: 'root',
//     database: 'demo',
...
app.get('/detail/:id', (req, res) => {
    requestInit(req, res);
    let sql = `select * from tb_stock_entry_detail where entryId=${mysql.escape(req.params.id)}`
...
app.get('/entry/:id', (req, res) => {
    requestInit(req, res);
    let sql = `select * from tb_stock_entry where entryId=${mysql.escape(req.params.id)}`
    let query = connect.query(sql, (err, result) => {
        console.log(query.sql);
        if (err) {
            console.log("操作失败" + err);
...
app.get('/server/getById/:id', (req, res) => {
    requestInit(req, res);
...
app.get('/server/pagination/:num/:size', (req, res) => {//分页查询
    requestInit(req, res);
    let sql = `select * from deviceinfo limit ?,?`//?防止sql注入,let query=connect.query会自动调用connect.escape([parm1,param2])
...
app.post('/server/add', (req, res) => {
    ...
    let sql = 'insert into deviceinfo set ?';//?防止sql注入,let query=connect.query会自动调用escape(req.body)
